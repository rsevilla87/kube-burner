...
test-k8s:
	...
	git remote -v
	cd test && KUBE_BURNER=$(TEST_BINARY) bats $(shell test/get_changed_labels.py test/bats_test_mappings.yml) -F pretty -T --print-output-on-failure test-k8s.bats -j $(JOBS)
