...
var prePullNs string = "kube-burner"
...
func PrePullImages(job Executor) {
	...
	log.Infof("Pre-pulling: Sleeping for %v", job.Config.PrePullPeriod)
	time.Sleep(job.Config.PrePullPeriod)
	log.Info("Pre-pulling: Deleting namespace")
	CleanupNamespaces(ClientSet, v1.ListOptions{LabelSelector: fmt.Sprintf("kubernetes.io/metadata.name=%s", prePullNs)})
}
...
func createDS(imageList []string, jobName string) error {
	...
	if err := createNamespace(ClientSet, prePullNs, map[string]string{}); err != nil {
		log.Fatal(err)
	}
	...
	log.Infof("Pre-pulling: Creating DaemonSet %s in namespace %s", dsName, prePullNs)
	_, err := ClientSet.AppsV1().DaemonSets(prePullNs).Create(context.TODO(), &ds, v1.CreateOptions{})
	if err != nil {
		return err
	}
	return nil
